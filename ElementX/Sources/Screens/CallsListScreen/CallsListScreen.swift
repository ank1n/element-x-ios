                    ...
                    ScrollView(.horizontal, showsIndicators: false) {
                        HStack(spacing: 8) {
                            ForEach(CallFilter.allCases, id: \.self) { filter in
                                ...
                                // STMOB-283: капсулу рисуем мы сами, поэтому системный
                                // стиль кнопки отключаем. Со включённой настройкой
                                // «Универсальный доступ → Формы кнопок» iOS подкладывает
                                // под кнопку СВОЮ форму, и поверх нашей капсулы появлялась
                                // вторая, серая — фильтры выглядели обведёнными (скрин с
                                // телефона Алёны 20.08). В «Чатах» этого нет, потому что
                                // там фильтр — Toggle со своим стилем, а не голая кнопка.
                                .buttonStyle(.plain)
                            }
                        }
                        .padding(.horizontal, 16)
