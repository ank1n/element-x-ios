...
class CallScreenViewModel: CallScreenViewModelType, CallScreenViewModelProtocol {
    ...
    private func endCall() async {
        ...
        isEndingCall = true
        // STMOB-297: снимаем взвод системного окна ПЕРВЫМ делом.
        // Гашение по состоянию соединения приходит слишком поздно: пользователь
        // завершает звонок, сворачивает приложение — и система успевает поднять
        // окно раньше, чем до нас доедет отбой. Снаружи это выглядело так: окно
        // появилось с пустым полем и именем собеседника и закрылось через секунду.
        CallPictureInPictureManager.shared.disarm()
