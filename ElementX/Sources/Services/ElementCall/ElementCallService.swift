...
class ElementCallService: NSObject, ElementCallServiceProtocol, PKPushRegistryDelegate, CXProviderDelegate {
    ...
    func provider(_ provider: CXProvider, perform action: CXAnswerCallAction) {
        ...
        // STMOB-279: без этой строки факт подтверждения соединения не наблюдался
        // ниоткуда — в логе были постановка звонка и активация аудио, а был ли
        // звонок объявлен соединённым, приходилось выводить по коду. Счётчик
        // длительности в «Динамическом острове» и в «Недавних» зависит ровно от неё.
        DiagLog.write("Call", "CallKit: входящий подтверждён соединённым → счётчик длительности пошёл")
