...
class ClientProxyMock: ClientProxyProtocol, @unchecked Sendable {
    ...
    var forceRefreshUnderlyingCallsCount = 0
    var forceRefreshCallsCount: Int {
        get {
            if Thread.isMainThread {
                return forceRefreshUnderlyingCallsCount
            } else {
                var returnValue: Int? = nil
                DispatchQueue.main.sync {
                    returnValue = forceRefreshUnderlyingCallsCount
                }
        ...
        set {
            if Thread.isMainThread {
                forceRefreshUnderlyingCallsCount = newValue
            } else {
                DispatchQueue.main.sync {
                    forceRefreshUnderlyingCallsCount = newValue
                }
    ...
    var forceRefreshCalled: Bool {
        return forceRefreshCallsCount > 0
    }
    var forceRefreshClosure: (() async -> Void)?

    func forceRefresh() async {
        forceRefreshCallsCount += 1
        await forceRefreshClosure?()
    }
    ...
    func createRoom(name: String, topic: String?, accessType: CreateRoomAccessType, isEncrypted: Bool, isSpace: Bool, userIDs: [String], avatarURL: URL?, aliasLocalPart: String?) async -> Result<String, ClientProxyError> {
...
class JoinedRoomProxyMock: JoinedRoomProxyProtocol, @unchecked Sendable {
    // STMOB-222: hand-added mock for hasMessageHistory() (regenerated by Sourcery normally).
    var hasMessageHistoryClosure: (() async -> Bool)?
    func hasMessageHistory() async -> Bool {
        await hasMessageHistoryClosure?() ?? false
    }
...
class TimelineProxyMock: TimelineProxyProtocol, @unchecked Sendable {
    ...
    // STMOB-222: hand-added mock for latestEventID() (regenerated by Sourcery normally).
    var latestEventIDClosure: (() async -> String?)?
    func latestEventID() async -> String? {
        await latestEventIDClosure?() ?? nil
    }
